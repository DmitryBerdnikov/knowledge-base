...
async function exportTscDiagnostics(outputPath) {
    ...
    const averagesOutput = 
        `\nAverage Metrics extends interfaces:\n` +
...
function runTscDiagnostics() {
    return new Promise((resolve, reject) => {
        exec('tsc ./src/interfaces.ts --diagnostics --noEmit', (error, stdout, stderr) => {
            if (error) {
...
const filePath = './tsc-diagnostics-interfaces.txt' // Укажите желаемый путь для сохранения файла
